...
b_obj = create_bucket_object('testawesomeproject2', tmp_file.name, key_prefix='temp/')
#tmp_file.unlink()
#b_obj = create_bucket_object('testawesomeproject2', tmp_file, key_prefix='temp/')
#print(tmp_file.exists())
#print(tmp_file.open().read())
c_obj=list(get_bucket('testawesomeproject2').object_versions.all())
#print(c_obj)

for _ in range(3):
    obj = create_bucket_object('testawesomeproject2',file_path=create_tempfile(),key_prefix='others/')
    print(f'Object {obj.key} created!')
    
bang=list(get_bucket('testawesomeproject2').objects.filter(Prefix='temp/'))
'''
print(list_buckets())

#get_bucket('testawesomeproject2').objects.filter(Prefix='temp/').delete()